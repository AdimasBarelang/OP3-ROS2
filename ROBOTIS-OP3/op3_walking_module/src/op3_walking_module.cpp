...
{
  queue_thread_ = std::thread(std::bind(&WalkingModule::queueThread, this));
  ...
  param_path_ = ros_node->get_parameter("walking_param_path").as_string();
...
void WalkingModule::queueThread()
{
  ...
  rclcpp::Service<op3_walking_module_msgs::srv::GetWalkingParam>::SharedPtr get_walking_param_server =
      ros_node->create_service<op3_walking_module_msgs::srv::GetWalkingParam>(
          "/robotis/walking/get_params",
          std::bind(&WalkingModule::getWalkigParameterCallback, this, std::placeholders::_1, std::placeholders::_2));

  /* sensor topic subscribe */
  auto walking_command_sub = ros_node->create_subscription<std_msgs::msg::String>(
      "/robotis/walking/command", 1,
  ...
  auto walking_param_sub = ros_node->create_subscription<op3_walking_module_msgs::msg::WalkingParam>(
      "/robotis/walking/set_params", 1,
      std::bind(&WalkingModule::walkingParameterCallback, this, std::placeholders::_1));

  executor.add_node(ros_node);

  rclcpp::WallRate loop_rate(1000 / control_cycle_msec_);
  while (rclcpp::ok())
  {
    executor.spin_some();
    loop_rate.sleep();
...
bool WalkingModule::getWalkigParameterCallback(const std::shared_ptr<op3_walking_module_msgs::srv::GetWalkingParam::Request> req,
                            ...
                            std::map<std::string, double> sensors)
{
  ...
  else if (walking_state_ == WalkingReady || walking_state_ == WalkingEnable)
  {
    ...
    if (walking_state_ == WalkingEnable && err_total > 5.0)
    {
      ...
      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "x_offset: " << walking_param_.init_x_offset);;

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "y_offset: " << walking_param_.init_y_offset);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "z_offset: " << walking_param_.init_z_offset);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "roll_offset: " << walking_param_.init_roll_offset * RADIAN2DEGREE);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "pitch_offset: " << walking_param_.init_pitch_offset * RADIAN2DEGREE);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "yaw_offset: " << walking_param_.init_yaw_offset * RADIAN2DEGREE);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "hip_pitch_offset: " << walking_param_.hip_pitch_offset * RADIAN2DEGREE);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "period_time: " << walking_param_.period_time * 1000);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "dsp_ratio: " << walking_param_.dsp_ratio);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "step_forward_back_ratio: " << walking_param_.step_fb_ratio);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "foot_height: " << walking_param_.z_move_amplitude);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "swing_right_left: " << walking_param_.y_swap_amplitude);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "swing_top_down: " << walking_param_.z_swap_amplitude);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "pelvis_offset: " << walking_param_.pelvis_offset * RADIAN2DEGREE);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "arm_swing_gain: " << walking_param_.arm_swing_gain);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "balance_hip_roll_gain: " << walking_param_.balance_hip_roll_gain);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "balance_knee_gain: " << walking_param_.balance_knee_gain);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "balance_ankle_roll_gain: " << walking_param_.balance_ankle_roll_gain);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "balance_ankle_pitch_gain: " << walking_param_.balance_ankle_pitch_gain);

      RCLCPP_WARN_STREAM_ONCE(rclcpp::get_logger("walking_module"), "balance : " << (walking_param_.balance_enable ? "TRUE" : "FALSE"));
...
void WalkingModule::loadWalkingParam(const std::string &path)
{
  ...
  {
    RCLCPP_ERROR(rclcpp::get_logger("yaml_file_loader"), "Fail to load yaml file.");;
    return;
  }
